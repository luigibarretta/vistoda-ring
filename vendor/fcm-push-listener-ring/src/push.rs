    ...
    fn decode(eckey: &EcKeyComponents, auth_secret: &[u8], bytes: &[u8]) -> Result<Self, Error> {
        ...
        for field in message.app_data {
            match field.key.as_str() {
                "crypto-key" => {
                    kex = URL_SAFE
                        .decode(padded_header_value(&field.value, "dh")?)
                ...
                "encryption" => {
                    salt = URL_SAFE
                        .decode(padded_header_value(&field.value, "salt")?)
...
fn padded_header_value(value: &str, name: &'static str) -> Result<String, Error> {
    ...
    let selected = value
        ...
        .ok_or(Error::MissingCryptoMetadata(name))?;
    if !selected.is_ascii() || selected.len() > 1024 {
        return Err(Error::MissingCryptoMetadata(name));
    }
    let mut padded = selected.to_owned();
...
mod tests {
    ...
    fn web_push_headers_are_named_bounded_and_padded() {
        let dh = padded_header_value("p256ecdsa=peer; dh=abc", "dh")
            .unwrap_or_else(|error| panic!("dh: {error}"));
        let salt =
            padded_header_value("salt=de", "salt").unwrap_or_else(|error| panic!("salt: {error}"));
        assert_eq!((dh.as_str(), salt.as_str()), ("abc=", "de=="));
        assert!(padded_header_value("keyid=none", "dh").is_err());
        assert!(padded_header_value("dh=", "dh").is_err());
        assert!(padded_header_value(&format!("dh={}", "x".repeat(1_025)), "dh").is_err());
        assert!(padded_header_value("dh=non-ascii-è", "dh").is_err());
    }
}
