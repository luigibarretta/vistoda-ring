...
impl Session {
    async fn request(
        ...
        let request = contract::AndroidCheckinRequest {
            ...
            checkin: contract::AndroidCheckinProto {
                r#type: Some(2), // Android device type (not Chrome)
                ..Default::default()
            },
            ..Default::default()
        };
    ...
    pub async fn request_token(&self, app_id: &str) -> Result<String, Error> {
        ...
        params.insert("app", "com.ringapp"); // Ring Android app package name
...
impl CheckedSession {
    ...
    /// Return the refreshed session material so callers can persist token rotation.
